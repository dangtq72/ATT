...
// THEM MOI MUC DICH VUA CHECK SESSION TIME OUT VUA CHECK LUON QUYEN TRUY CAP 
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data != null && data["Code"] == -1) {
                    alert(data["Msg"]);
                    window.location.href = "/home/voting"
...
// cap nhat lai Layout khi duoc chon 
...
function CheckDateArlert(date) {
    var matches = /^(\d{2})[-\/](\d{2})[-\/](\d{4})$/.exec(date);
    if (matches == null) {
        alert("Ngày nhập vào không đúng định dạng dd/mm/yyyy"); return false;
    }
    var m = matches[2];
    var d = matches[1];
    var y = matches[3];
...
function checkDateCompare(p_date1, p_date2, p_compare_type, p_msgErr, p_Tag) {
    ...
    switch (p_compare_type) {
        ...
        case 99: // thêm mới check nhỏ hơn hoặc = loại luôn 
            {
                if (date1 <= date2) {
                    var _Err = " <div id=\"divErr\" style=\"color:red ;padding-top:15px;clear:both; padding-bottom:10px\">" + p_msgErr + "</div>";
                    var _tagHtml = "#" + p_Tag;
...
//    var html = "<div style=\"height: 100%; width: 100%; top: 0; left: 0; z-index: 1001; position: absolute;\"><div class=\"loader\"><div></div><div></div><div></div><div></div><div></div></div></div>";
//    if (is_display) {
//        $("body").append(html);
//    }
//    else {
...
///HungTD: kiểm tra file tải lên có phải là ảnh ko
function validateExtension_Img(v) {
...
//Check đúng định dạng time hh:mm
function isTime_hhmm(strTime) {
    
    var rxTimePattern = /^(0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/;
    var dtArray = strTime.match(rxTimePattern); // is format OK?
    if (dtArray == null)
        return false;
    else
        return true;
