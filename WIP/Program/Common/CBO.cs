...
namespace NVS_Common
{
    public class CBO
    {
                            ...
                            try
                            {
                                ...
                                switch (pType.FullName)
                                {
                                    ...
                                    case "System.Decimal":
                                        ...
                                        //1/9/2015 LinhNN rem lại vì khi tostring vì dữ liệu là 2,2E-5 khi convert sẽ bị lối
                                        ((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, Convert.ToDecimal(dr[_possition]), null);
                                        break;
                                    case "System.Int16":
                                        // ((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, Convert.ToInt16(dr[_possition].ToString()), null);
                                        //1/9/2015 LinhNN rem lại vì khi tostring vì dữ liệu là 2,2E-5 khi convert sẽ bị lối
                                        ((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, Convert.ToInt16(dr[_possition]), null);
                                        break;
                                    case "System.Int32":
                                        // ((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, Convert.ToInt32(dr[_possition].ToString()), null);
                                        //1/9/2015 LinhNN rem lại vì khi tostring vì dữ liệu là 2,2E-5 khi convert sẽ bị lối
                                        ((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, Convert.ToInt32(dr[_possition]), null);
                                        break;
                                    case "System.Int64":
                                        //((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, Convert.ToInt64(dr[_possition].ToString()), null);
                                        //1/9/2015 LinhNN rem lại vì khi tostring vì dữ liệu là 2,2E-5 khi convert sẽ bị lối
                                        ((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, Convert.ToInt64(dr[_possition]), null);
                                        break;
                                    case "System.DateTime":
                                        ((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, (DateTime)dr[_possition], null);
                                        break;
                                    case "System.Double":
                                        //((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, Convert.ToDouble(dr[_possition].ToString()), null);
                                        //1/9/2015 LinhNN rem lại vì khi tostring vì dữ liệu là 2,2E-5 khi convert sẽ bị lối
                                        ((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, Convert.ToDouble(dr[_possition]), null);
                                        break;
                                    case "System.Guid":
                                        ((PropertyInfo)objProperties[_fieldname]).SetValue(objObject, new Guid(dr[_possition].ToString()), null);
                                        break;
                                    default:
                                        // try explicit conversion
        ...
        public static ArrayList FillCollectionFromDataTable(DataTable dt, Type objType)
        {
            try
            {
                ArrayList objFillCollection = new ArrayList();
        ...
        private static object CreateObject(Type objType, DataRow dr, ArrayList objProperties)
        {
            ...
            for (intProperty = 0; intProperty <= objProperties.Count - 1; intProperty++)
            {
                if (((PropertyInfo)objProperties[intProperty]).CanWrite)
                {
                    ...
                    try
                    {
                        //sangdd rem vao ngay 14.08.2015 tranh th bi exception se rat lau sua bang pos 1.2
                        //object obj = dr[((PropertyInfo)objProperties[intProperty]).Name];
                        //hasDatarowColum = true;


                        //sangdd sua ngay :14.08.2015
                        //muc dich : kiem tra neu trong datarow co thi moi lay ra tranh th bi exception 
                        //theo doi xem co bi loi ko  ?
                        // pos 1.2
                        if (dr.Table.Columns.Contains(((PropertyInfo)objProperties[intProperty]).Name))
                        {
                            object obj = dr[((PropertyInfo)objProperties[intProperty]).Name];
                            hasDatarowColum = true;
                        }
        ...
        /// <summary>
        /// sangdd them moi ham return list de hien thi len view.cshtml return ve IEnumerable
        /// đỡ phải convert từ arraylist to list 
        /// </summary>
        /// <param name="ds">dataset </param>
        /// <param name="objType">objectinfo [lưu ý khai báo trong objectinfo các trường phải giống hệt trong database ]</param>
        /// <returns></returns>
        public static List<TSource> Fill2ListFromDataSet<TSource>(DataSet ds, Type objType)
        {
            try
            {
                var objFillCollection = new List<TSource>();
                object objFillObject = null;
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    // add to collection
                    TSource obj = (TSource)objFillObject;
                    objFillCollection.Add(obj);
                }
        ...
        #region Kiểm NH- doc lít từ datatable

        /// <summary>
        /// Kiểm NH thêm hàm đọc list từ datatable
        /// </summary>
        /// <typeparam name="TSource"></typeparam>
        /// <param name="ds"></param>
        /// <param name="objType"></param>
        /// <returns></returns>
        public static List<TSource> Fill2ListFromDataTable<TSource>(DataTable dt, Type objType)
        {
            try
            {
                var objFillCollection = new List<TSource>();
        ...
        //sangdd them
        public static object FillObjectFromDataTable(DataTable dt, Type objType)
        {
            ...
            try
            {
                ...
                {
                    // fill business object
                    objFillObject = CreateObjectFromDataSet(objType, dt.Rows[0], objProperties, arrOrdinals);
                }
                else
                {
                    objFillObject = null;
